...
export async function apiCall<T>(
  ...
  if (Platform.OS !== "web") {
    ...
    if (sessionToken) {
      ...
      debugLog("[API] Authorization header added");
    }
  } else {
    debugLog("[API] apiCall:", {
      endpoint,
  ...
  try {
    ...
    if (contentType && contentType.includes("application/json")) {
      ...
      debugLog("[API] JSON response received");
      return data as T;
    }

    const text = await response.text();
    debugLog("[API] Text response received");
...
export async function exchangeOAuthCode(
  ...
  debugLog("[API] exchangeOAuthCode called");
  // Use GET with query params
  const params = new URLSearchParams({ code, state });
  const endpoint = `/api/oauth/mobile?${params.toString()}`;
  debugLog("[API] Calling OAuth mobile endpoint:", endpoint);
  ...
  debugLog("[API] OAuth exchange result:", {
...
export async function establishSession(token: string): Promise<boolean> {
  try {
    debugLog("[API] establishSession: setting cookie on backend...");
    const baseUrl = getApiBaseUrl();
    ...
    debugLog("[API] establishSession: cookie set successfully");
    return true;
  } catch (error) {
