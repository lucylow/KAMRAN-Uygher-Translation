...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  try {
    ...
    throw new Error(`LLM invoke returned invalid JSON: ${String(error)}`);
  }

  if (!Array.isArray(result.choices)) {
    throw new Error("LLM invoke returned an invalid response payload.");
  }
...
export async function listLLMModels(): Promise<ModelsResponse> {
  ...
  try {
    result = (await response.json()) as ModelsResponse;
  } catch (error) {
    throw new Error(`List LLM models returned invalid JSON: ${String(error)}`);
  }

  if (!Array.isArray(result.data)) {
    throw new Error("List LLM models returned an invalid models payload.");
  }
