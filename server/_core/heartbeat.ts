...
const callForge = async <T>(
  ...
  try {
    ...
    const message =
      error instanceof DOMException && error.name === "TimeoutError"
        ? `Heartbeat ${rpc} request timed out after ${HEARTBEAT_REQUEST_TIMEOUT_MS}ms`
        : `Heartbeat ${rpc} network error: ${String(error)}`;
    throw new TRPCError({
      code: "INTERNAL_SERVER_ERROR",
      message,
    });
  ...
  try {
    ...
    throw new TRPCError({
      code: "INTERNAL_SERVER_ERROR",
      message: `Heartbeat ${rpc} returned invalid JSON: ${String(error)}`,
    });
  }
};
